...
export default function OrdersManagementPage() {
  const {
    ...
    today, dateFrom, dateTo, searchTerm,
    setPage, setDateFrom, setDateTo,
  ...
  return (
    <Dashboard>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input type="search"
              ...
              className="pl-9 pr-4 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 w-full"
            />
          </div>
      ...
      {completeResult && (
        <CompleteResultModal result={completeResult} onClose={clearCompleteResult} />
      )}
    </Dashboard>
  );
}
  ...
  return (
    <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4 flex items-center justify-between">
      ...
      <div className="flex gap-3 ml-4">
        {onRetry && <button onClick={onRetry} className="text-sm underline text-red-700 dark:text-red-300">Retry</button>}
    ...
    <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
      <div className="flex gap-3">
        ...
        <p className="text-sm text-blue-700 dark:text-blue-300">
          ...
          Upload inventory, then click <strong>Mark Complete</strong> to fulfill remaining items.
        </p>
      </div>
    </div>
  );
...
function TabButton({ active, onClick, label, count, badgeVariant }: {
  active: boolean; onClick: () => void; label: string;
  count: number; badgeVariant: "gray" | "orange";
}) {
...
function Pagination({ page, totalPages, total, onPrev, onNext }: {
  ...
  return (
    <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
      <p className="text-sm text-gray-500 dark:text-gray-400">Page {page} of {totalPages} ({total} orders)</p>
